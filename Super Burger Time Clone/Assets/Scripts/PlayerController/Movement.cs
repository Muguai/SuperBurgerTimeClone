...
public class Movement : BoxColliderCasts
{
	...
	[HideInInspector] public bool descendSlope = false;
	[HideInInspector] public bool SlopeHitLeft;
	...
	public void Move(Vector2 displacement, Vector2 input)
	{
		...
		objectInput = input;
		debugText.text = "(" + (displacement.x * 1000).ToString("F1") + " " + (displacement.y * 1000).ToString("F1") + ")";
	...
	void CheckVerticalCollisions(ref Vector2 displacement)
	{
        ...
        for (int i = 0; i < verticalRayCount; i++)
        {
            ...
            if (hit)
            {
                ...
                if (hit.collider.tag == "Through" || hit.collider.tag == "BurgerPart")
                {
                    ...
                    if (objectInput.y == -1 && hit.collider.tag != "BurgerPart")
                    {
	...
	void SlideDownMaxSlope(RaycastHit2D hit, ref Vector2 displacement)
	{
		...
		if (slopeAngle > maxSlopeAngle && slopeAngle < wallAngle - wallTolerence)
		{
			...
			displacement.x = (Mathf.Sign(hit.normal.x) * (Mathf.Abs(displacement.y) - hit.distance) / Mathf.Tan(slopeAngle * Mathf.Deg2Rad)) * maxSlopeSpeed;
